...
async function testAdminAuthRateLimiting() {
  ...
  await test("Blocks after limit exceeded", async () => {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (lastRes.status === 429) {
        return {
          pass: true,
          detail: `Blocked after ${i + 1} requests. Retry-After: ${
  ...
  await test("429 response has correct format", async () => {
    ...
    if (res.status !== 429) {
      return {
        pass: true,
        detail: "Not rate limited yet (previous test may have failed)",
      };
    }
...
async function main() {
  ...
  console.log(
    `${colors.blue}║    Rate Limiting Test Suite              ║${colors.reset}`
  );
  console.log(
  ...
  if (failed === 0) {
    ...
    console.log(
      `${colors.dim}  Restart the server to reset in-memory rate limits.${colors.reset}\n`
    );
  }
