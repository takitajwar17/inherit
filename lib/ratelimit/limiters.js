 ...
 * AI Endpoint Limiter
 * 
 * Protects against excessive AI API usage (Groq).
 * 20 requests per minute per IP.
 * 
 * Use for: /api/voice-routing
 */
...
export const limiterConfigs = {
  'admin-auth': { requests: 5, windowMs: 15 * MINUTE, description: 'Admin login attempts' },
  'quest-attempt': { requests: 10, windowMs: MINUTE, description: 'Quest attempt creation' },
  'quest-submit': { requests: 12, windowMs: MINUTE, description: 'Quest answer submission' },
  'ai-endpoint': { requests: 20, windowMs: MINUTE, description: 'AI API calls (user-based if authenticated, IP-based if anonymous)' },
  'youtube-api': { requests: 30, windowMs: MINUTE, description: 'YouTube searches (user-based if authenticated, IP-based if anonymous)' },
  'question-reply': { requests: 10, windowMs: MINUTE, description: 'Question replies' },
  'vote': { requests: 30, windowMs: MINUTE, description: 'Upvotes/downvotes' },
  'admin-quest': { requests: 20, windowMs: MINUTE, description: 'Admin quest operations' },
