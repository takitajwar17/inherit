...
export class GeneralAgent extends BaseAgent {
  ...
  async process(message, context = {}) {
    const { history = [], language = "en", userName } = context;
    ...
    try {
      // Build personalized system prompt if userName is available
      let personalizedPrompt = this.systemPrompt;
      
      if (userName && typeof userName === 'string' && userName.trim() && userName !== 'there') {
        personalizedPrompt = `${this.systemPrompt}

IMPORTANT: The user's name is ${userName}. When greeting them or responding, use their name naturally to make the conversation personal and friendly. For example: "Hello ${userName}!" or "Hi ${userName}!"`;
        
        logger.info("Using personalized prompt with userName", {
          userName: userName,
          promptIncludesName: personalizedPrompt.includes(userName),
        });
      } else {
      ...
      logger.debug("Built conversation messages", {
        ...
        systemPromptPreview: messages[0]?.content?.substring(0, 150),
      });
