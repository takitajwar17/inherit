...
export const createRoadmapTool = new DynamicStructuredTool({
  name: "create_roadmap",
  description:
    "Create a new learning roadmap for a CS/programming topic. Use this when the user asks to 'create a roadmap for...', 'make me a learning path for...', or 'help me learn...'. The roadmap will be structured with phases and tasks, including documentation links and YouTube video recommendations.",
  schema: z.object({
    title: z
      .string()
      .min(3)
      .max(100)
      .describe("Title of the roadmap (e.g., 'Learn React in 30 Days')"),
    ...
    phases: z
      .array(
        z.object({
          name: z
            .string()
            .describe("Phase name (e.g., 'Foundation', 'Core Concepts')"),
          tasks: z
            .array(
              z.object({
                topic: z.string().describe("The learning task/topic"),
                documentation: z
                  .string()
                  .optional()
                  .describe(
                    "URL to official documentation or tutorial article"
                  ),
                youtubeVideoId: z
                  .string()
                  .optional()
                  .describe(
                    "YouTube video ID for a recommended tutorial (e.g., 'dQw4w9WgXcQ')"
                  ),
                videoDuration: z
                  .string()
                  .optional()
                  .describe("Video duration (e.g., '15:30', '1:02:45')"),
              })
            )
            .describe(
              "List of specific learning tasks with optional resources"
            ),
          duration: z
            .string()
            .describe("Estimated duration (e.g., '1 week', '3 days')"),
        })
      )
      .min(2)
      .max(8)
      .describe("Learning phases with tasks, resources, and duration"),
    description: z
      .string()
      .optional()
      .describe("Brief description of the roadmap"),
  }),
    ...
    try {
      ...
      for (const phase of phases) {
        for (const task of phase.tasks) {
          steps.push({
            step: stepNumber++,
            topic: typeof task === "string" ? task : task.topic,
            description: `Part of ${phase.name} phase`,
            phase: phase.name,
            duration: phase.duration,
            completed: false,
            // Add resource links if provided
            documentation: typeof task === "object" ? task.documentation : null,
            videoId: typeof task === "object" ? task.youtubeVideoId : null,
            videoDuration: typeof task === "object" ? task.videoDuration : null,
          });
        }
      }
      ...
      return JSON.stringify({
        ...
        roadmap: {
          ...
          difficulty,
          description:
            description || `A ${difficulty} roadmap to learn ${topic}`,
          phases: phases,
...
export const getUserRoadmapsTool = new DynamicStructuredTool({
  ...
  description:
    "Get a list of the user's existing learning roadmaps. Use this when the user asks 'show me my roadmaps', 'what roadmaps do I have', or wants to see their learning paths.",
  schema: z.object({
    status: z
      .enum(["all", "in-progress", "completed"])
      .default("all")
      .describe("Filter by roadmap status"),
    limit: z
      .number()
      .default(10)
      .describe("Maximum number of roadmaps to return"),
  }),
  func: async ({ status, limit }, config) => {
    try {
      ...
      return JSON.stringify({
        ...
        count: roadmapList.length,
        message: `Found ${roadmapList.length} roadmap${
...
export const getRoadmapDetailsTool = new DynamicStructuredTool({
  ...
  description:
    "Get detailed information about a specific roadmap including all phases and steps. Use this when the user asks about a particular roadmap's content.",
  schema: z.object({
  ...
  func: async ({ roadmapId }, config) => {
    try {
      ...
      const completedSteps = (roadmap.content?.steps || []).filter(
        (s) => s.completed
      ).length;
...
export const updateRoadmapProgressTool = new DynamicStructuredTool({
  ...
  description:
    "Mark a step in a roadmap as completed. Use this when the user says they've finished a topic or completed a learning step.",
  schema: z.object({
    ...
    completed: z
      .boolean()
      .default(true)
      .describe("Whether the step is completed"),
  }),
  func: async ({ roadmapId, stepNumber, completed }, config) => {
    try {
      ...
      const stepIndex = roadmap.content?.steps?.findIndex(
        (s) => s.step === stepNumber
      );
      ...
      roadmap.content.steps[stepIndex].completedAt = completed
        ? new Date()
        : null;
      ...
      logger.error("Update roadmap progress tool error", {
        error: error.message,
      });
      return JSON.stringify({
        success: false,
