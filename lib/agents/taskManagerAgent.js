...
export class TaskManagerAgent extends BaseAgent {
  ...
  async process(message, context = {}) {
    ...
    logger.info("TaskManagerAgent received context", {
      hasClerkId: !!clerkId,
      clerkId: clerkId,
    ...
    if (!clerkId) {
      logger.warn("TaskManagerAgent called without clerkId - cannot create tasks");
      return this.formatResponse(
        language === "bn"
          ? "দুঃখিত, আপনাকে লগ ইন করতে হবে টাস্ক তৈরি করতে।"
          : "Sorry, you need to be logged in to create tasks. Please log in and try again.",
    ...
    try {
      ...
      // Add full user context summary if available
      if (contextSummary) {
        fullContext += `\n\n--- USER'S FULL PROFILE ---\n${contextSummary}\n--- END PROFILE ---`;
      }
      ...
      if (clerkId) {
        ...
        if (pendingTasks.length > 0) {
          fullContext += `\n\nUser's current pending tasks (${pendingTasks.length} total):\n${pendingTasks
            .map(
              (t, i) =>
                `${i + 1}. "${t.title}" - Priority: ${t.priority}, Category: ${t.category || 'general'}, Due: ${
                  t.dueDate ? t.dueDate.toLocaleDateString() : "no date set"
                }, ID: ${t._id}`
            )
            .join("\n")}`;
        } else {
      ...
      if (response.tool_calls && response.tool_calls.length > 0) {
        ...
        let finalContent = this.safeExtractContent(finalResponse);
        
        // Ensure we always have content - use tool output summary if model response is empty
        if (!finalContent || finalContent.trim() === "") {
          finalContent = toolOutputs || "Task operation completed successfully.";
          logger.warn("TaskManagerAgent: Model returned empty response, using tool output", {
      ...
      let content = this.safeExtractContent(response);
      
      // Ensure we always have content
      if (!content || content.trim() === "") {
        content = language === "bn" 
          ? "আমি আপনার অনুরোধ বুঝতে পারিনি। আবার চেষ্টা করুন।"
          : "I couldn't understand that request. Could you please rephrase?";
      }
