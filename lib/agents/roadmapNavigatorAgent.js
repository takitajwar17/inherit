...
export class RoadmapNavigatorAgent extends BaseAgent {
  ...
  async process(message, context = {}) {
    ...
    try {
      ...
      await connect();
      const roadmaps = await Roadmap.find({ clerkId }).limit(5).sort({ updatedAt: -1 });
      ...
      if (roadmaps.length > 0) {
        ...
        for (const r of roadmaps) {
          ...
          fullContext += `\n- "${r.title}" (ID: ${r._id}): ${completed}/${total} steps, ${progress}% complete, status: ${r.status || 'active'}`;
        }
      } else {
