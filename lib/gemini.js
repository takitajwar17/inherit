...
function getNextApiKey() {
  ...
  for (let i = 0; i < keyStatus.length; i++) {
    ...
    if (status.isRateLimited && status.rateLimitResetTime && now > status.rateLimitResetTime) {
  ...
  const earliestReset = keyStatus.reduce((min, status) => 
    !min || (status.rateLimitResetTime && status.rateLimitResetTime < min.rateLimitResetTime) 
      ? status 
      : min
  , null);
  ...
  if (earliestReset) {
    ...
    logger.warn(`All API keys rate limited. Using key ${earliestReset.index + 1}, wait time: ${Math.round(waitTime / 1000)}s`);
...
export function createGeminiModel(type = "main") {
  ...
  try {
    ...
    logger.debug("Gemini model created successfully", { type, keyIndex: index + 1 });
